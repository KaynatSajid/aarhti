function CartItem({ cartItem, onRemove, onIncrement, onDecrement }) {
  ...
  const handleIncrement = () => {
    ...
    if (crop.quantity > 0) {
      ...
      } else alert("Maximum " + crop.quantity + " items can be added");
    }
  };
  ...
  console.log('image wali',crop.image[0]);
  console.log('image wali',crop.price);
  return (
    <li className="shopping-cart-sidebar-item">
      <div className="shopping-cart-sidebar-item-image">
        {Array.isArray(crop.image) ? ( 
           <img src={crop.image[0].data} alt={crop.name} /> 
        ) : (
          <img src={crop.image.data} alt={crop.name} />
        )}
      </div>
